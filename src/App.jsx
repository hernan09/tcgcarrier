...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-zinc-100 antialiased">
          ...
          <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-3 sm:px-6">
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="inline-flex items-center gap-1.5 rounded-xl border border-emerald-500/20 bg-emerald-500/10 px-3 py-1.5 sm:px-4 text-xs sm:text-sm font-medium text-emerald-300 transition-all hover:bg-emerald-500/20 hover:border-emerald-500/30 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-400/50 shrink-0"
              >
                <svg className="h-3.5 w-3.5 sm:h-4 sm:w-4 shrink-0" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
      ...
      {showMap ? (
        <WorldMap onBack={handleBackFromMap} />
      ) : showGame ? (
        <GameDemo onBack={handleBackFromGame} />
      ) : (
        <Home onStartGame={handleStartGame} onStartMap={handleStartMap} />
      )}
    </div>
  )
}
